...
//var url = 'https://docs.google.com/uc?id=0B4MinpbTTsM-UWVhekNyLWliOUk&export=download';
...
storeApp.controller('storeController', function ($scope, $http, $filter, $routeParams, $location, DataService, $sce, CONFIG) {
    $scope.dataLoaded = false;
    $scope.selectedCurrency="Rs"
    ...
    $scope.fetch = function() {
    	...
    	for (var i = 0; i < $scope.cart.items.length; i++) {
    		...
    		arrItems.push(item.sku);
    	}
    	
		$http.post('download.php', { "data" : arrItems }).
		success(function(data, status) {
			//$scope.cartstatus = status;
		...
		error(function(data, status) {
			$scope.cartstatus = " Error " + data + "Err Code " + status  || "Request failed";			
		});
    ...
    DataService.store.getProducts().then(function (data) {
        ...
        for (var i = 0, len = $scope.products.length; i < len; i++) {
            ...
            //if (prod.storeid == "7cc6cb94-0938-4675-b84e-6b97ada53978") {
                if (prod.imagename.length < 1) {
                    prod.imagename =  prod.PRODUCTS_FOLDER + "/" + "nopic.png";
                }
                if (prod.carousel) {
       ...
       // $scope.search(); // arvind warning
        ////////////////////////////////////////////////////////////////////////
       // $scope.vchr_date = new Date($scope.product.expecteddate); // arvind warning
...
storeApp.factory('DataService', function ($http, $q, CONFIG) {
   ...
   //var tmpData =  CONFIG.CF_DATA_FILE ;
    ...
    function Store() {
       ...
       // $.getJSON(tmpData, function (data, status, headers, config) {
            var products = [];
...
storeApp.directive('myYoutube', function ($sce) {
    return {
        ...
        template: '<div class="video"><iframe ng-src="{{url}}" frameborder="0" allowfullscreen></iframe></div>',
        link: function (scope) {
            console.log('here');
...
storeApp.directive('embedVideo', function ($sce) {
    return {
        ...
        scope: { tube: '=', code: '=' },
        replace: true,
        template: '<div class="video"><iframe ng-src="{{url}}" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe></div>',
        link: function (scope) {
            //console.log('here');
      ...
      return function Dialog(templateUrl/*optional*/, options, passedInLocals) {
        ...
        '</div>';
          ...
          /* modalBody + */
                    ...
                    '<img ng-src="{{$imagename}}" alt="{{imagename}}" class="photopopup" style="max-width:280px;" ng-click="close()" />' +
                    '</div>' +
